...
```{r}
votes <- read_csv("data_538/election-forecasts-2020/presidential_ev_probabilities_2020.csv", col_types = cols(
  cycle = col_double(),
...
ggplot(votes, aes(total_ev, evprob_chal)) + geom_col()
```
...
```{r}
votes %>% 
  select(c(evprob_challenger, total_ev)) %>% 
  filter(total_ev>300)
```
